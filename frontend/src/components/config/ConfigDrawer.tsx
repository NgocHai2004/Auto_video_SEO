...
import ImageUpload from '../pipeline/ImageUpload';
import { DEFAULT_PIPELINE_CONFIG, RESOLUTION_PRESETS, PRIVACY_OPTIONS } from '../../utils/constants';
import type { PipelineConfig, ExecutionMode } from '../../types';

interface ConfigDrawerProps {
  isOpen: boolean;
  onToggle: () => void;
  onStart: (image: File, config: PipelineConfig) => void;
  mode: ExecutionMode;
  isRunning: boolean;
  hasStarted: boolean;
}
...
export default function ConfigDrawer({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onStart(image, config);
  };

  const canSubmit = image && config.description.trim() && !isRunning && !hasStarted;
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="Product or scene description..."
                  rows={3}
                  className="w-full bg-slate-800/80 border border-slate-700 rounded-lg px-3 py-2 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/30 resize-none transition-colors"
                />
              </div>
              ...
              <AnimatePresence>
                {showAdvanced && (
                  ...
                  >
                    ...
                    {/* Script Override */}
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500">Custom Audio Script</label>
                      <textarea
                        value={config.audio_script || ''}
                        onChange={(e) => update('audio_script', e.target.value || null)}
                        placeholder="Leave empty to auto-generate..."
                        rows={2}
                        className="w-full bg-slate-800/80 border border-slate-700 rounded px-2 py-1 text-[10px] text-slate-200 placeholder-slate-600 resize-none focus:outline-none focus:border-violet-500"
                      />
                    </div>

                    {/* Video Prompt Override */}
                    <div className="space-y-1">
                      <label className="text-[10px] text-slate-500">Custom Video Prompt</label>
                      <textarea
              ...
              >
                {mode === 'auto' ? '⚡ Prepare & Run All' : '🎯 Prepare for Manual Execution'}
              </button>
            </form>
          </motion.div>
        )}
